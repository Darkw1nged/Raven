...
import me.darkwinged.raven.struts.Achievement;
import me.darkwinged.raven.struts.Quest;
...
//                "&7Achievements Completed: &d" + Utils.formatAmount(user.get().stream().filter(Achievement::isCompleted).count()),
//                "&7Quests Completed: &d" + Utils.formatAmount(user.getQuests().stream().filter(Quest::isCompleted).count()),
                "&7Raven Coins: &d" + Utils.formatAmount(user.getRavenCoins()),
                "&7Shadow Essence: &d" + Utils.formatAmount(user.getShadowEssence())
        )));
